...
function ViewModal({ record, onClose }) {
  ...
  const fields = [
    ['Testador',          record.testador],
    ['Data do Testamento', record.data_testamento],
    ['Ano',              record.ano],
    ['Livro',            record.livro],
    ['Folha',            record.folha],
    ['Tabelião',         record.tabeliao],
    ['Testemunhas',      record.testemunhas],
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div className={`bg-white rounded-xl shadow-2xl w-full flex flex-col max-h-[92vh] ${isImage ? 'max-w-5xl' : 'max-w-lg'}`}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0">
          <h2 className="font-semibold text-slate-800">Visualizar Testamento</h2>
          ...
          <div className={`overflow-y-auto p-5 ${isImage ? 'w-72 flex-shrink-0' : 'flex-1'}`}>
            <div className="grid grid-cols-2 gap-3">
              {fields.map(([label, val]) => val ? (
                <div key={label} className={label === 'Testador' || label === 'Testemunhas' ? 'col-span-2' : 'col-span-1'}>
...
export default function TestamentosList() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await testamentosApi.list(params);
  ...
  async function handleDelete(id) {
    ...
    setDeleting(id);
    try {
      await testamentosApi.remove(id);
  ...
  const novoUrl    = livroId ? `/testamentos/novo?livro_id=${livroId}` : '/testamentos/novo';
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Testamentos</h1>
      ...
      <div className="relative mb-4">
        ...
        <input
          type="text"
          placeholder="Buscar por testador, tabelião, município..."
          value={search}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-5 py-3.5 font-semibold text-slate-600">Testador</th>
              ...
              {records.map((r, i) => (
                <tr key={r.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${i % 2 === 0 ? '' : 'bg-slate-50/30'}`}>
                  <td className="px-5 py-3.5 font-medium text-slate-800">
                    {r.testador || <span className="text-slate-400 italic">Não informado</span>}
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <button title="Editar" onClick={() => navigate(`/testamentos/${r.id}/editar`)}
