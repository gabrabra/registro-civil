...
const EMPTY = {
  livro_id: '', vendedor: '', cpf_vendedor: '', comprador: '', cpf_comprador: '',
  data_escritura: '', ano: '', livro: '', folha: '',
  descricao_imovel: '', endereco_imovel: '', valor: '',
  tabeliao: '', cartorio: '', municipio: '', estado: '', observacoes: ''
};
...
export default function EscriturasForm() {
  const { id }         = useParams();
  ...
  useEffect(() => {
    if (!isEdit) return;
    escriturasApi.get(id).then(r => {
      setForm({
        livro_id:        r.livro_id        ? String(r.livro_id) : '',
        vendedor:        r.vendedor        || '',
        cpf_vendedor:    r.cpf_vendedor    || '',
        comprador:       r.comprador       || '',
        cpf_comprador:   r.cpf_comprador   || '',
        data_escritura:  r.data_escritura  || '',
        ano:             r.ano             ? String(r.ano) : '',
        livro:           r.livro           || '',
        folha:           r.folha           || '',
        descricao_imovel: r.descricao_imovel || '',
  ...
  async function handleProcess() {
    ...
    try {
      ...
      setForm(prev => ({
        ...
        cpf_comprador:   reg.cpf_comprador   || '',
        data_escritura:  reg.data_escritura  || '',
        ano:             reg.ano             ? String(reg.ano) : '',
        livro:           reg.livro           || '',
        folha:           reg.folha           || '',
        descricao_imovel: reg.descricao_imovel || '',
        endereco_imovel:  reg.endereco_imovel  || '',
        valor:           reg.valor           || '',
  ...
  function validate() {
    const e = {};
    if (!form.vendedor?.trim()) e.vendedor = 'Obrigatório';
    if (!form.comprador?.trim()) e.comprador = 'Obrigatório';
    return e;
  }
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      navigate(livroId ? `/escrituras-compra-venda?livro_id=${livroId}` : '/escrituras-compra-venda');
    } catch (err) {
  ...
  const backUrl = searchParams.get('livro_id')
    ? `/escrituras-compra-venda?livro_id=${searchParams.get('livro_id')}`
    : '/escrituras-compra-venda';
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-semibold text-slate-900">
            {isEdit ? 'Editar Escritura' : 'Nova Escritura de Compra e Venda'}
          </h1>
          <p className="text-sm text-slate-500">Registro de Imóveis</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          <h2 className="text-sm font-semibold text-slate-700 border-b border-slate-100 pb-2">Partes</h2>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Vendedor" name="vendedor" value={form.vendedor} onChange={set} confidence={confidence} error={errors.vendedor} required />
            <Field label="CPF do Vendedor" name="cpf_vendedor" value={form.cpf_vendedor} onChange={set} confidence={confidence} />
            <Field label="Comprador" name="comprador" value={form.comprador} onChange={set} confidence={confidence} error={errors.comprador} required />
            <Field label="CPF do Comprador" name="cpf_comprador" value={form.cpf_comprador} onChange={set} confidence={confidence} />
          </div>
        </div>

        {/* Escritura */}
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          <h2 className="text-sm font-semibold text-slate-700 border-b border-slate-100 pb-2">Dados da Escritura</h2>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <Field label="Data da Escritura" name="data_escritura" value={form.data_escritura} onChange={set} confidence={confidence} className="col-span-2" />
            <Field label="Ano" name="ano" value={form.ano} onChange={set} confidence={confidence} />
            <Field label="Valor" name="valor" value={form.valor} onChange={set} confidence={confidence} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Cartório" name="cartorio" value={form.cartorio} onChange={set} confidence={confidence} />
          </div>
        </div>

        {/* Imóvel */}
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          <h2 className="text-sm font-semibold text-slate-700 border-b border-slate-100 pb-2">Imóvel</h2>
          <Field label="Endereço do Imóvel" name="endereco_imovel" value={form.endereco_imovel} onChange={set} confidence={confidence} />
          <Field label="Descrição do Imóvel" name="descricao_imovel" value={form.descricao_imovel} onChange={set} confidence={confidence} textarea />
